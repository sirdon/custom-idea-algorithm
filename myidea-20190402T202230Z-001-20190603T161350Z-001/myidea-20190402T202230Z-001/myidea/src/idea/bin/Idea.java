...
//private static final int     rounds = 2;         // number of rounds
    
static int rd;
static int keyconstant=0;
...
public void crypt (byte[] data, int dataPos) {    
   ...
   for (int round = 0; round < rd; round++) {
      int y0 = madd(x0, subKey[p++]);
      int y1 = add(x1, subKey[p++]);
      int y2 = add(x2, subKey[p++]);
      int y3 = madd(x3, subKey[p++]);
      //
      int t0 = madd(y0 ^ y2, subKey[p++]);
      int t1 = add(y1 ^ y3, t0);
      int t2 = madd(t1, subKey[p++]);
      int t3 = add(t0, t2);
      //
      x0 = y0 ^ t2;
      x1 = y2 ^ t2;
      ...
      x3 = y3 ^ t3; }
   //
   int r0 = madd(x0, subKey[p++]);
   int r1 = add(x2, subKey[p++]);
   int r2 = add(x1, subKey[p++]);
   int r3 = madd(x3, subKey[p++]);
   //
...
private static int madd (int a, int b) {
   return (a + b+keyconstant) & 0xFFFF; }
// Additive Inverse.
// The argument and the result are within the range 0 .. 0xFFFF.
private static int addInv (int x) {
   return (0x10000 - x) & 0xFFFF; }

private static int maddInv (int x) {
   return (0x10000 - x) & 0xFFFF; }
...
/*private static int invaddInv (int x) {
   if (x <= 1) {
      return x; }
   int y = 0x10001;
   int t0 = 1;
...
private static int[] invertSubKey (int[] key) {
   ...
   int p = 0;
   int i = rd * 6;
   invKey[i + 0] = maddInv(key[p++]+keyconstant);
   invKey[i + 1] = addInv(key[p++]);
   invKey[i + 2] = addInv(key[p++]);
   invKey[i + 3] = maddInv(key[p++]+keyconstant);
   for (int r = rd - 1; r >= 0; r--) {
      i = r * 6;
      int m = r > 0 ? 2 : 1;
      int n = r > 0 ? 1 : 2;
      invKey[i + 4] =        key[p++];
